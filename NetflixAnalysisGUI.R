...
dataset1 <- read.csv("dataset01_clear.csv")
str(dataset1)

# Ajusta o tipo de dado de algumas colunas
dataset1$Subscribers.Q4.2021 <- as.numeric(gsub(",", "", dataset1$Subscribers.Q4.2021))
dataset1$Revenue.Q4.2021 <- as.numeric(gsub(",", "", dataset1$Revenue.Q4.2021))
...
genre <- read.csv("dataset02_clear.csv")
tree <- read.csv("dataset03_clear.csv")
countries <- read.csv("dataset06_clear.csv")
                          ...
                          sidebarLayout(
                            sidebarPanel(                              
                              selectInput("select", 
                                          ...
                                          choices = list("Faturamento da Netflix Q4-2021" = "Revenue.Q4.2021", 
                                                         "Assinaturas da Netflix Q4-2021" = "Subscribers.Q4.2021",
                          ...
                          sidebarLayout(
                            sidebarPanel(
                              selectInput("select3", 
                                          ...
                                          choices = list("Faturamento Netflix Q4-2021" = "Revenue.Q4.2021", 
                                                         "Assinaturas Netflix Q4-2021" = "Subscribers.Q4.2021"),
                                          selected = 1),
...
server <- function(input, output) {
  ...
  output$barPlot <- renderPlotly({
    ...
    fig <- fig %>% add_trace(y = ~standard_premiuma_diff, name = 'Premium')
  ...
  output$mapscatPlot <- renderPlotly({
    ...
    fig <- plot_ly(data = dfms, x = ~Subscribers.Q4.2021, y = ~Revenue.Q4.2021, type = "scatter", mode = "markers", text = ~Country)
